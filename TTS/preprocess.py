...
from .config import data_path, sample_rate, outputs_per_step, cleaners, num_mels
...
class LJDatasets(Dataset):
    ...
    def __init__(self, csv_file, root_dir):
        ...
        """
        nrows = t.load(data_path+'/.n_samples')
        if nrows < 0 :
            self.landmarks_frame = pd.read_csv(csv_file, sep='|', header=None)
        else : 
            self.landmarks_frame = pd.read_csv(csv_file, sep='|', header=None, nrows = nrows)
    ...
    def __getitem__(self, idx):
        ...
        text = np.asarray(text_to_sequence(text, [cleaners]), dtype=np.int32)
        mel = np.load(wav_name[:-4] + '.pt.npy')
        mel_input = np.concatenate([np.zeros([1,num_mels], np.float32), mel[:-1,:]], axis=0)
