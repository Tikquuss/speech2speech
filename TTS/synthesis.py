...
from .config import checkpoint_path, cleaners, sample_path, sr
from .utils import spectrogram2wav
from .network import ModelPostNet, Model

def load_checkpoint(step, model_name="transformer"):
    state_dict = t.load(checkpoint_path+'/checkpoint_%s_%d.pth.tar'% (model_name, step))   
...
def synthesis(text, args):
    ...
    text = np.asarray(text_to_sequence(text, [cleaners]))
    ...
    write(sample_path + "/test.wav", sr, wav)
